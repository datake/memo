...
def main
  ...
  for a in 1 .. a_node do
    for b in 1 .. b_node do
...
def print_transgraphs (p_node,a_node,b_node,output_filename)
  ...
  File.open(output_filename, "w") do |io|
    ...
    sets_of_nodes.each{|set_of_nodes|
      ...
      #if a_set.size==a_node && b_set.size==b_node.to_i
      ...
      sets_of_nodes.each{|search_nodes|
        ...
        if (set_of_nodes[0] & search_nodes[0]).size + (set_of_nodes[1] & search_nodes[1]).size >0
          ...
          #arrayの和集合は|で表現
          if (set_of_nodes[0] | search_nodes[0] | set_of_nodes[1] | search_nodes[1]).size==(a_node+b_node)
            if (set_of_nodes[0] | set_of_nodes[1]).size > 1 #マルダンのアプリケーション通過する条件
              #pivot1について
